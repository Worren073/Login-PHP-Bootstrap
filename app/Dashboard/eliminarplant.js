/**
 * Modal para confirmar la eliminación de una planta
 */
...
/**
 * Función para eliminar una planta desde la modal
 */
async function eliminarplanta(id, planta) {
    try {
        ...
        document.getElementById("confirmDeleteBtn").addEventListener("click", async function () {
            var idPlanta = this.getAttribute("data-id");

            try {
                const response = await axios.post("../db/eliminar_planta.php", { id: idPlanta });

                if (response.status === 200 && response.data.status === "success") {
                    // Eliminar la fila correspondiente a esta planta de la tabla
                    document.querySelector(`#planta_${idPlanta}`).remove();
                    
                    // Mostrar mensaje de éxito
                    if (window.toastrOptions) {
                        toastr.options = window.toastrOptions;
                        toastr.success("¡La planta se eliminó correctamente!");
                    }
                } else {
                    alert(`Error al eliminar la planta con ID ${idPlanta}`);
                }
            } catch (error) {
                console.error(error);
