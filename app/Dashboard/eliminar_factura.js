/**
 * Modal para confirmar la eliminación de una Factura
 */
 ...
 */
async function eliminarfactura(id, factura) {
    try {
        ...
        document.getElementById("confirmDeleteBtn").addEventListener("click", async function () {
            ...
            try {
                const response = await axios.post("../db/eliminar_factura.php", { id: idFactura });

                if (response.status === 200 && response.data.status === "success") {
                    // Eliminar la fila correspondiente a esta Factura de la tabla
                    document.querySelector(`#factura_${idFactura}`).remove();
                    
                    // Mostrar mensaje de éxito
                    if (window.toastrOptions) {
                        toastr.options = window.toastrOptions;
                        toastr.success("¡La Factura se eliminó correctamente!");
                    }
                } else {
                    alert(`Error al eliminar la Factura con ID ${idFactura}`);
                }
            } catch (error) {
                console.error(error);
